...
The C<ScopeManager> interface abstracts both the activation of L<Span> instances
...
Set the specified L<Span> as the active instance for the current context
(usually a thread).

    my $span = $tracer->start_span( 'some operation', ignore_active_span => 1 );
    
    my $manager = $tracer->get_scope_manager;
    
    my $scope = $manager->activate_span( $span, finish_span_on_close => 0 );

The returned L<Scope> represents the active state for the span. Once its active
...
A C<Bool> when set to false, the <Span> will not be automatically finished when
the L<Scope> has been closed. This is 'true' by default.
...
Return the currently active L<Scope> which can be used to access the
currently active L<Span>, using C<$scope->get_span>.
...
The L<Scope> that is currently active.
