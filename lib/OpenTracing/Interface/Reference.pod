...
Creates a C<SpanContext> C<Reference> for which L<type_is_child_of> is 'true'.

    Reference->new_child_of( $span_context );
...
The C<SpanContext> the returned C<Reference> should should refer to.
...
A newly instantiated C<Reference>


=head2 new_follows_from( $span_context )

Creates a C<SpanContext> C<Reference> for which L<type_is_follows_from> is
'true'.

    Reference->new_follows_from( $span_context );
...
instantiated with the C<new_child_of> constructor.

    say "I'm a CHILD_OF reference"
        if $reference->type_is_child_of;
...
instantiated with the C<new_follows_from> constructor.

    say "I'm a FOLLOWS_FROM reference"
        if $reference->type_is_child_of;
